...
class LSTMEnergyEnv:
    def __init__(self):
        ...
        self.lstm = nn.LSTM(self.observation_space, self.hidden_space ,self.num_layers, batch_first=True)
    ...
    def reset(self):
        ...
        self.E_max = max(self.solar_radiation) * 0.15
        self.P_max = max(self.solar_radiation) * 0.15
        ...
        return observation, self.h, self.c    
   
    def step(self, action,h,c):
        ...
        lstm_input = torch.tensor(observation, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # バッチ次元とシーケンス次元を追加
        lstm_output, (h_next, c_next) = self.lstm(lstm_input, (h, c))
        ...
        return observation, reward, done, h_next,c_next
